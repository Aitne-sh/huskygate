...
**IMPORTANT**: Do NOT use heredoc syntax or pipe into a shell variable command. Use file redirection (`< file`) instead.
...
python3 -c '
import json, os
_payload_path = os.path.join(os.environ.get('TMPDIR', '/tmp'), "gmail_payload.json")
payload = {
...
with open(_payload_path, "w") as f:
    json.dump(payload, f)
'
bash "$RUNNER" < "${TMPDIR:-/tmp}/gmail_payload.json"
```
...
**Error codes from Step 1 (compose_email.py):**
...
**NEVER silently retry non-transient errors. Report them to the user immediately.**

If the MCP tool call in Step 2 fails with an authentication error, tell the user to verify their Google Workspace MCP server configuration.
