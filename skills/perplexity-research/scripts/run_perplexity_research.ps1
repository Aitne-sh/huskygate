...
<#
.SYNOPSIS
  Perplexity Research runner wrapper script (Windows).
  Reads JSON from stdin, delegates to run_perplexity_research.py via the skill venv,
  and outputs structured JSON results to stdout.

.USAGE
  ConvertTo-Json @{query="What is quantum computing?"; mode="search"} |
    Set-Content "$env:TEMP\pplx_payload.json"
  powershell -NoProfile -File run_perplexity_research.ps1 < "$env:TEMP\pplx_payload.json"
  powershell -NoProfile -File run_perplexity_research.ps1 --check
  powershell -NoProfile -File run_perplexity_research.ps1 --version
...
$RUNNER_PY = Join-Path $SCRIPT_DIR 'run_perplexity_research.py'
...
function Ensure-Venv {
  ...
  try {
    ...
    if ((Test-Path $REQUIREMENTS) -and (Select-String -Path $REQUIREMENTS -Pattern '^\s*[^#\s]' -Quiet)) {
...
if ($args.Count -gt 0 -and $args[0] -eq '--check') {
  ...
  if (-not (Test-Path $RUNNER_PY)) {
    $errors += "run_perplexity_research.py not found at $RUNNER_PY"
  }
  ...
  # Check API key
  $hasApiKey = [bool]$env:PERPLEXITY_API_KEY
  ...
  if (-not $hasApiKey) {
    $errors += 'PERPLEXITY_API_KEY is not set. Configure it in the dashboard Skills settings.'
  }
...
# ── Main: delegate to run_perplexity_research.py ─────────────────────────

if (-not (Test-Path $RUNNER_PY)) {
  Write-JsonError "run_perplexity_research.py not found at $RUNNER_PY" 'SCRIPT_NOT_FOUND'
  exit 0
}
...
$tmpPayload = Join-Path $env:TEMP "pplx_payload_$(Get-Random).json"
try {
