...
- Do NOT pipe into a shell variable command (e.g. `| "$PY" ...`). Use file redirection (`< file`) instead.
...
with open(".tmp/pplx_payload.json", "w") as f:
    json.dump(payload, f)
'
"$PY" "$RUNNER" < .tmp/pplx_payload.json
```
